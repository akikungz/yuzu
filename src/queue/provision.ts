...
export class ProvisionQueueWorker {
	private readonly logger = rootLogger.child({ service: "provision-worker" });
		...
		this.worker = new Worker(
			...
			async (job) => {
				...
				try {
					...
					await this.useCase.markFailed(job.data.instanceId, error.message);
	...
	public async instanceProvisionJob(
		instanceId: number,
		userId: number,
