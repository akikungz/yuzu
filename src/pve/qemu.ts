import { logger } from "@yuzu/infrastructure/observability/logger";
...
export const cloneQemu = async (
	...
	const cloneRes = await pveApi.POST(
...
export const resizeQemuDisk = async (
	...
	// Resize the disk
	const resizeDiskRes = await pveApi.PUT(
...
export const editQemu = async (
	...
	const editRes = await pveApi.PUT(
...
export const getQemuStatus = async (node: string, vmid: number) => {
	const statusRes = await pveApi.GET(
...
export const setQemuStatus = async (
	...
	const statusRes = await pveApi.POST(
	...
	return new Promise<void>((resolve, reject) => {
		...
		const agentInterval = setInterval(async () => {
			...
			if (elapsed > maxWaitTime) {
				...
				reject(
					new Error(
						`QEMU Guest Agent not responding after ${(maxWaitTime / 1000).toFixed(0)}s for VM ${vmid}`,
					),
				);
				return;
			}
			...
			try {
				const agentRes = await pveApi.GET(
...
export const setQemuGuestUserPassword = async (
	...
	const setPasswordRes = await pveApi.POST(
	...
	throw new Error(
		`Failed to set guest password for VM ${vmid} user ${username}.`,
	);
};
...
export const setQemuGuestHostname = async (
	...
	const setHostnameRes = await pveApi.POST(
