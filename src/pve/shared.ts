...
export const upidStatusCheck = async (
	...
	const taskLog = logger.child({
		node,
		upid: upid.slice(-20),
		operation: "upid-check",
	});
	...
	return new Promise<UPIDStatusResult>((resolve, reject) => {
		...
		const upidInterval = setInterval(async () => {
			...
			if (elapsed > maxWaitTime) {
				...
				taskLog.error(
					{ elapsed: `${elapsed.toFixed(0)}ms`, pollCount },
					"Task monitoring timed out",
				);
				reject(
					new Error(
						`UPID task timed out after ${(maxWaitTime / 1000).toFixed(0)}s: ${upid}`,
					),
				);
				return;
			}
			...
			try {
				...
				if (statusRes.data?.data) {
					...
					taskLog.error(
						{ response: statusRes },
						"Failed to get task status - empty response",
					);
					reject(
						new Error(
							`Failed to get status for UPID ${upid} on node ${node}: empty response`,
						),
					);
				}
			} catch (err) {
...
export const getNodeWithLeastLoad = async (): Promise<string> => {
	...
	if (nodesRes.data?.data) {
		let onlineNodes = nodesRes.data.data.filter(
...
export const listQemuVmsAllNodes = async (): Promise<VmidWithNode[]> => {
	...
	if (nodesRes.data?.data) {
		...
		for (const node of nodesRes.data.data) {
			...
			if (vmsRes.data?.data) {
				allVms.push(
					...vmsRes.data.data.map((vm) => ({ vmid: vm.vmid, node: node.node })),
				);
			}
		}
 ...
 */
export const generateNextVmid = async (
	prevId?: number,
	base: number = 1000,
): Promise<number> => {
